...
class Data:
    def __init__(self, csv_loader) -> None:
        ...
        self.dvl_velocities = self.loaded_csvs["dvl_data"][["velocity.x", "velocity.y", "velocity.z"]].to_numpy().T
        self.imu_angular_velocities = self.loaded_csvs["imu_data"][["angular_velocities.x", "angular_velocities.y", "angular_velocities.z"]].to_numpy().T
        self.imu_linear_accelerations = self.loaded_csvs["imu_data"][["linear_acceleration.x", "linear_acceleration.y", "linear_acceleration.z"]].to_numpy().T
        self.vicon_positions = self.loaded_csvs["vicon_data"][["translation.x", "translation.y", "translation.z"]].to_numpy().T
        self.vicon_orientation = self.loaded_csvs["vicon_data"][["rotation.w",
            "rotation.x", "rotation.y", "rotation.z"]].to_numpy().T
...
def plot_traj(time_stamps, calibration_time, ground_truth, dead_reckoning, seconds_to_plot=120):
    ...
    lc = LineCollection(segments, cmap="viridis", linewidth=2, label="Dead Reckoning", zorder=1)
    ...
    lc_gt = LineCollection(segments_gt, linestyles='--', cmap="viridis", linewidth=2, label="Ground Truth")
    # set color to date values
    lc_gt.set_array(time_stamps[first_point_ground_truth:last_point_ground_truth])
    ...
    # Mark end points
    ax.plot(ground_truth[0, last_point_ground_truth-1],
...
def main():
    ...
    experiment_name = experiments[10]
